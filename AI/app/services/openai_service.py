...
def generate_report_comments(
    ...
    각 코멘트 작성 공통 규칙:
    - 가독성을 위해 내용(성분, 제품, 타이밍 등)이 바뀔 때마다 반드시 2줄바꿈(\\n\\n)으로 문단을 명확히 분리하세요.
    - 강조를 위해 핵심 단어(성분명, 제품명, 섭취시간 등) 앞뒤에 마크다운 볼드체 기호(**)를 붙여 출력하세요(예: **마그네슘**, **아침 식후**).
    - 수치를 말할 때 500mg 등 정확한 숫자를 쓰지 말고, "절반 정도 채웠어요", "전혀 섭취하지 않네요", "조금 부족해요" 등 직관적이고 둥근 정성적 표현을 쓰세요.

    세부 코멘트 규칙:
    1. excess_comment: 과잉 성분이 있을 경우 위 공통 규칙에 맞춰 초과 정도와 감량 안내. 없으면 "과잉 섭취된 성분이 없어 정말 다행이에요! 앞으로도 건강하게 관리해봐요!"로 짧게 작성.
    2. deficiency_comment: 위 공통 규칙에 따라 부족 성분별로 건강에 미치는 영향을 부드럽게 안내.
    3. product_comment: 추천 제품별로 이유(성분 함량, 흡수율, 조합 장점 등)를 공통 규칙에 맞춰 설명. 아예 안 먹던 성분이라면 그 점도 짚어줌.
    4. schedule_comment: 섭취 타이밍 배치 이유를 작성할 때, 단일 성분(비타민C, 칼슘 등) 단위로 문장을 쪼개지 말고, 반드시 사용자가 섭취 중인 **영양제 무리(alias: 종합비타민 등) 단위로 그룹화**하여 설명. "종합비타민은 ㅇㅇ에 드시면 좋아요. 종합비타민에 포함된 ㅁㅁ 성분은 ㅇㅇ에 드실 때 흡수가 잘 되기 때문이에요." 처럼 묶어서 하나의 흐름으로 설명. 이 때 'absorption_notes'와 'interaction_notes'를 근거로 활용.
